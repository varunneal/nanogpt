...
Fused Attention
===============
Accessed from https://github.com/triton-lang/triton/blob/main/python/tutorials/06-fused-attention.py

Credits: OpenAI kernel team, Triton developers
Minor changes by @varunneal
...
"""
If N_CTX <= 128 and so block_size (BLOCK_M * BLOCK_N) is less than 128 x 128 = 16384,
num_warps = 8 is PRUNED and num_warps = 4 is forced.

Additionally BLOCK_M is chosen to be less then N_CTX. 

For N_CTX > 128, best settings of 128 x 128 block size and num_warps = 8 chosen. 
"""
configs = [
...
attention(q, k, v, causal, sm_scale, warp_specialize):
    q, k, v: (batch_size, heads, seqlen, head_dim)
    - head_dim must be in {16, 32, 64, 128, 256}

    Inputs can be bf16, fp16, fp8_e5m2.
"""
